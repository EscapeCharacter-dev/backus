...
bool_t KscParseType(KscType *type)
{
	...
	case KSC_KEYWORD_CHAR:
		type->kind = KSC_TYPE_SBYTE;
		break;
	case KSC_KEYWORD_UCHAR:
		type->kind = KSC_TYPE_BYTE;
		break;
	case KSC_KEYWORD_SHORT:
		type->kind = KSC_TYPE_SHORT;
		break;
	case KSC_KEYWORD_USHORT:
		type->kind = KSC_TYPE_USHORT;
		break;
	case KSC_KEYWORD_INT:
		type->kind = KSC_TYPE_INT;
		break;
	case KSC_KEYWORD_UINT:
		type->kind = KSC_TYPE_UINT;
		break;
	case KSC_KEYWORD_LONG:
		type->kind = KSC_TYPE_LONG;
		break;
	case KSC_KEYWORD_ULONG:
		type->kind = KSC_TYPE_ULONG;
		break;
	case KSC_KEYWORD_HALF:
		type->kind = KSC_TYPE_HALF;
		break;
	case KSC_KEYWORD_FLOAT:
		type->kind = KSC_TYPE_FLOAT;
		break;
	case KSC_KEYWORD_DOUBLE:
		type->kind = KSC_TYPE_DOUBLE;
		break;
	case KSC_KEYWORD_VOID:
